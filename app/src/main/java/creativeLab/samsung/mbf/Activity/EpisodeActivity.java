...
public class EpisodeActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        adapter = new EpisodeListAdapter(this, animationList);
    ...
    private void launchFingerPrintScreen() {
        startActivity(new Intent(EpisodeActivity.this, FingerprintActivity.class));
        //finish();
    }
    ...
    private void prepareAnimation() {
        ...
        String[] episode_name = new String[1024];
        String[] episode_id = new String[1024];
        String[] episode_image = new String[1024];
        int episode_num = 0;
        Animation episode_card_ui;

        try {
            String jsonFileLocation = AssetJSONFile("json/episode_list.json", this);
            JSONObject jsonObject = new JSONObject(jsonFileLocation);
            JSONArray jsonArray = jsonObject.getJSONArray("episode_lists");
            for (int i = 0; i < jsonArray.length(); i++) {
                JSONObject jsonSubObject = (JSONObject) jsonArray.get(i);
                String episodeName = jsonSubObject.getString("episode_name");
                String episodeId = jsonSubObject.getString("episode_id");
                String episodeImage = jsonSubObject.getString("episode_thumbnail");

                covers[i] = this.getResources().getIdentifier(episodeImage, "drawable", this.getPackageName());

                episode_name[i] = episodeName;
                episode_id[i] = episodeId;
                episode_image[i] = episodeImage;

                episode_card_ui = new Animation(episode_name[i], i, covers[i]);
                episode_num++;
                animationList.add(episode_card_ui);
                // Log.d("TAG", "episode_name " + episode_name);
            }
        } catch (IOException e) {
            e.printStackTrace();
